...
* Introduction to `cluster.fork()`, worker processes and communication from the master thread and 0MQ's interprocess communication (IPC). **The problem is that this doesn't work in Windows (POSIX-like interprocess communication is not supported)**